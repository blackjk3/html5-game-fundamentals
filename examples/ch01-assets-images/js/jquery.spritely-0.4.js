				...
				var animate = function(el) {
					...
					if (options.rewind == true) {
						...
						if ($._spritely.instances[el_id]['current_frame'] >= frames.length - 1) {
							...
							// Check to see if we are dealing with more than 1 row.
							if(options.rows > 1) {
								$._spritely.instances[el_id]['row']++;
								el.spState($._spritely.instances[el_id]['row']);
							
								if($._spritely.instances[el_id]['row'] > options.rows) {
							  		$._spritely.instances[el_id]['row'] = 1;
								}
							}
						} else {
							if(options.rows > 1 && options.lastRowCount > 0) {
								if($._spritely.instances[el_id]['row'] == options.rows && $._spritely.instances[el_id]['current_frame'] == options.lastRowCount-1){
									$._spritely.instances[el_id]['current_frame'] = 0;
									$._spritely.instances[el_id]['row'] = 1;
									el.spState(1);
								} else {
